...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	...
	typedef int(*MakeSubFnPtr)(int);


	//declaring function pointer variable
	typedef int(*MakeSubFnPtr)(int);
	MakeSubFnPtr ptrFuncMakeSub = NULL;
	...
	switch (iMsg)
	{
		//load the library
	case WM_CREATE:
		hDll = LoadLibrary(TEXT("MyMathOneSub.dll"));
		if (hDll == NULL)
		{
		...
		//this is step 2b
		ptrFuncMakeSub = (MakeSubFnPtr)GetProcAddress(hDll, "MakeSub");
		if (ptrFuncMakeSub == NULL)
		{
