...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	...
	typedef int(*makedivfnptr)(int);
	
	//Declaration Function Pointer Variable
	
	makedivfnptr ptrfuncmakediv = NULL;
	int n1 =100,n2=10;
	int div;
	TCHAR str[255];
...
//Code
	switch (iMsg)
	{
		//Load the Library
	case WM_CREATE:
		hDll = LoadLibrary(TEXT("MyMathTwoDiv.dll"));
		if (hDll == NULL)
		{
		...
		ptrfuncmakediv = (makedivfnptr)GetProcAddress(hDll, "MakeDiv");
		if (ptrfuncmakediv == NULL)
		{
			MessageBox(hwnd, TEXT("LoadLibrary Fail"), TEXT("ERROR"), MB_OK);
			FreeLibrary(hDll);
			hDll = NULL;
		...
		div = ptrfuncmakediv(n1/n2);
		wsprintf(str, TEXT("Division of %d & %d is %d"), n1,n2, div);
